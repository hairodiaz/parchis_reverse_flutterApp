...
/// 🔐 AUTH SERVICE - Sistema de Autenticación Híbrido
/// 
/// Funcionalidades:
...
class AuthService {
  ...
  Future<void> _setupAuthListener() async {
    _auth.authStateChanges().listen((User? firebaseUser) async {
      ...
      if (firebaseUser != null) {
        print('🔐 Usuario Firebase autenticado: ${firebaseUser.email}');
        await _syncFirebaseUserWithLocal(firebaseUser);
      } else {
        print('🚪 Usuario Firebase desautenticado');
        await _handleLogout();
      }
    });
  }

  // 🔄 Sincronizar usuario Firebase con local
  Future<void> _syncFirebaseUserWithLocal(User firebaseUser) async {
    try {
      // Verificar si el usuario local ya existe y está registrado
      ...
      if (localUser != null && localUser.facebookId == firebaseUser.uid) {
        ...
        print('🔄 Usuario ya sincronizado: ${localUser.name}');
        return;
      }
      ...
      if (userDoc.exists) {
        ...
        print('📥 Usuario sincronizado desde Firestore: ${syncedUser.name}');
      } else {
  ...
  // 📤 Migrar usuario local a Firestore
  Future<void> _migrateLocalUserToFirestore(User firebaseUser, LocalUser? localUser) async {
    try {
      ...
      print('📤 Usuario migrado a Firestore: ${migratedUser.name}');
      
      // Si había usuario invitado, mostrar mensaje de migración exitosa
      if (localUser != null && localUser.isGuest) {
        print('🎉 ¡Datos de invitado migrados exitosamente!');
      }
    } catch (e) {
      print('❌ Error migrando usuario a Firestore: $e');
    }
  }
  ...
  Future<bool> signInWithFacebook() async {
    try {
      ...
      if (!await _hasInternetConnection()) {
        print('❌ Sin conexión a internet para login con Facebook');
        return false;
      }
      ...
      if (result.status == LoginStatus.success) {
        ...
        if (userCredential.user != null) {
          print('✅ Login con Facebook exitoso: ${userCredential.user!.email}');
          return true;
        }
      } else {
        print('❌ Login con Facebook cancelado: ${result.status}');
        return false;
      }
    } catch (e) {
  ...
  Future<bool> signInWithGoogle() async {
    try {
      // Verificar conectividad
      if (!await _hasInternetConnection()) {
        print('❌ Sin conexión a internet para login con Google');
        return false;
      }
  ...
  Future<void> logout() async {
    try {
      ...
      print('✅ Logout exitoso - Usuario invitado creado');
    } catch (e) {
  ...
  Future<void> _handleLogout() async {
    ...
    if (localUser == null || !localUser.isGuest) {
      // Crear usuario invitado si no hay uno o si el actual está registrado
      await HiveService.createGuestUser();
  ...
  // 📊 Sincronizar estadísticas con Firestore
  Future<void> syncStatsToFirestore() async {
    try {
      ...
      print('📊 Estadísticas sincronizadas con Firestore');
    } catch (e) {
  ...
  Future<void> updateNickname(String newNickname) async {
    try {
      ...
      // Actualizar en Firestore si está registrado
      if (_currentFirebaseUser != null && await _hasInternetConnection()) {
        await _firestore.collection('users').doc(_currentFirebaseUser!.uid).update({
          'name': newNickname,
          'lastUpdateAt': FieldValue.serverTimestamp(),
        });
        print('📝 Nickname actualizado en Firestore');
      }

      print('📝 Nickname actualizado: $newNickname');
    } catch (e) {
      print('❌ Error actualizando nickname: $e');
    }
  }
